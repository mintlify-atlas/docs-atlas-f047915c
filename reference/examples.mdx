...
public class WordCount {
    ...
    public static void main(String[] args) {
        ...
        // Settings — pass the input path as args[1]
...
public class WordCountOnKafkaTopic {
    ...
    public static void main(String[] args) {
        ...
        String inputTopicName  = "banking-tx-small-csv";
        String outputTopicName = "word_count_contribution___banking-tx-small-csv";

        if (args.length > 0) {
            inputTopicName = args[0];
        }
        ...
        JavaPlanBuilder planBuilder = new JavaPlanBuilder(wayangContext)
                .withJobName(String.format(
                        "WordCount using Java Context on Kafka topic (%s)", inputTopicName))
                .withUdfJarOf(WordCountOnKafkaTopic.class);

        // Build the WayangPlan.
        planBuilder
                // Read records from the input Kafka topic.
                .readKafkaTopic(inputTopicName).withName("Load data from topic")
                ...
                .writeKafkaTopic(
                        outputTopicName,
                        d -> String.format("%d, %s", d.getField1(), d.getField0()),
                        "job_test_1",
...
object kmeans {
  def main(args: Array[String]): Unit = {
    ...
    // Register both platforms — the optimizer picks per step.
    val wayangContext = new WayangContext(new Configuration)
      .withPlugin(Java.basicPlugin)
      .withPlugin(Spark.basicPlugin)
    ...
    case class TaggedPointCounter(x: Double, y: Double, cluster: Int, count: Long) {
      def add_points(that: TaggedPointCounter) =
        TaggedPointCounter(this.x + that.x, this.y + that.y, this.cluster, this.count + that.count)
    ...
    // Create k random initial centroids.
    ...
    // UDF: for each point, find the nearest centroid.
    // Centroids are passed in via broadcast rather than a join.
      ...
      override def apply(point: Point): TaggedPointCounter = {
        ...
        for (centroid <- centroids) {
          val distance = Math.pow(
            Math.pow(point.x - centroid.x, 2) + Math.pow(point.y - centroid.y, 2), 0.5
          )
