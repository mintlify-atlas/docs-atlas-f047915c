...
`MachineLearning.plugin()` acts as a coordination plugin for ML workflows. It registers the `MachineLearning` platform marker and enables ML-specific configuration properties. Actual ML operator implementations are contributed by `Spark.mlPlugin()` or by the `wayang-tensorflow` module.
